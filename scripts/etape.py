...
for i in range(1, 22):  # Étapes 1 à 21
    ...
    if response.status_code == 200:
        ...
        # Trouver le lien AJAX du classement d'étape
        tab_ranking = soup.find("span", class_="js-tabs-ranking", string="Classement de l'étape")
        if not tab_ranking:
        ...
        ajax_url = "https://www.letour.fr" + json_data["ite"]  # URL du classement d'étape

        # Requête AJAX pour obtenir le vrai classement d'étape
        ...
        if response_ajax.status_code == 200:
            ...
            # Trouver toutes les lignes de classement
            rows = ajax_soup.find_all("tr", class_="rankingTables__row")
            ...
            for row in rows:
                ...
                if classement and dossard and temps:
                    coureurs.append({
                        "no_classement": int(classement),
                        "id": int(dossard),
                        "temps": temps
                    })

            # Sauvegarde des résultats de l'étape
            fichier_json = f"resultats_etape_{i}.json"
